...
for norm in norm_layers:
    test_script.append(
        ...
        f"--direction BtoA --num_test {num_test} --epoch latest --results_dir {f'results/hyperparameters/hyperparameter_norm'} --norm {norm} --gpu_ids {gpu_ids} --checkpoints_dir {checkpoints_dir}"
    )
